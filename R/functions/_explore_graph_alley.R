...
explore_graph_alley <- function(r = r, data, var_type, var_left, var_right, df, 
  ...
  if (is_scale_in_df("borough_empty", df)) {
    alley[!is.na(alley$created),] |> 
      ...
      scale_x_continuous(name = curbcut::cc_t(lang = r$lang(), translation = translation, "Green alley start date")) +
      theme_minimal() +
    ...
    if (is.na(select_id)) {
      ...
      alley[!is.na(alley$created),] |> 
        ...
        scale_x_discrete(labels = labels,
                         name = curbcut::cc_t(lang = r$lang(), translation = translation, "Visited green alleys type")) +
        theme_minimal() +
        theme(text = element_text(family = "SourceSansPro", size = 12),
              legend.position = "none", 
              panel.grid.minor.x = element_blank(),
