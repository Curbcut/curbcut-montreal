...
explore_graph_alley <- function(r = r, data, var_type, var_left, var_right, df, 
  ...
  if (curbcut::is_scale_df("borough_empty", df)) {
    alley[!is.na(alley$created),] |> 
      ...
      scale_x_continuous(name = curbcut::cc_t(lang = r$lang(), "Green alley start date")) +
      theme_minimal() +
  ...
  # Show type-summary histogram or NULL in alley mode
    ...
    if (is.na(select_id)) {
      ...
      labels <- sapply(labels, cc_t, lang = r$lang(), USE.NAMES = FALSE)
      ...
      alley[!is.na(alley$created),] |> 
        ...
        scale_x_discrete(labels = labels,
                         name = curbcut::cc_t(lang = r$lang(), "Visited green alleys type")) +
        theme_minimal() +
        theme(text = element_text(family = "SourceSansPro", size = 12),
              legend.position = "none", 
              panel.grid.minor.x = element_blank(),
