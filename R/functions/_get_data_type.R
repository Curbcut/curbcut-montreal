...
get_data_type <- function(df, var_left, var_right, build_str_as_DA = TRUE) {
  ...
  if (build_str_as_DA && curbcut::is_scale_df("building", df) && length(var_right) == 2 && 
      var_right[1] == var_right[2]) return("building_NA_delta_bivar")
  if (build_str_as_DA && curbcut::is_scale_df("building", df) && length(var_left) == 2 && 
      var_left[1] == var_left[2]) return("building_NA_delta")
  
  if (build_str_as_DA && curbcut::is_scale_df("building", df) && length(var_left) == 1 
  ...
  if (curbcut::is_scale_df("raster", df)) return("q100")  
  if (curbcut::is_scale_df(c("heatmap", "point"), df)) return("point")
  if (curbcut::is_scale_df("qual", var_left[1])) return("qual")
  if (!curbcut::is_scale_df(c(all_choropleths, "grid"), df)) return(df)
