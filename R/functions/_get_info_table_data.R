...
get_info_table_data <- function(r = r, data, var_type, var_left, var_right, df, 
  ...
  if (!curbcut::is_scale_df("building", df)) build_str_as_DA <- FALSE
  ...
  if (curbcut::is_scale_df("date", df)) {
  ...
  var_left_label <- sapply(var_left_label, cc_t, lang = r$lang())
  var_right_label <- sapply(var_right_label, cc_t, lang = r$lang())
  
  out$title_left <- 
    curbcut::cc_t(lang = r$lang(), variables[variables$var_code == var_left,]$var_title)
  if (var_right != " ") out$title_right <- 
    curbcut::cc_t(lang = r$lang(), variables[variables$var_code == var_right,]$var_title)
  
  out$exp_left <- 
    curbcut::cc_t(lang = r$lang(), variables[variables$var_code == var_left,]$explanation)
  out$exp_right <- 
    curbcut::cc_t(lang = r$lang(), variables[variables$var_code == var_right,]$explanation)
  ...
  out$scale_sing <- curbcut::cc_t(lang = r$lang(), scale_sing)
  out$scale_plural <- curbcut::cc_t(lang = r$lang(), scale_plural)
  ...
  out$place_name <- curbcut::cc_t(lang = r$lang(), out$place_name)
  
  name_2 <- curbcut::cc_t(lang = r$lang(), select_name$name_2)
  out$place_heading <- 
    scales_dictionary$place_heading[scales_dictionary$scale == gsub(".*_", "", df)]
  out$place_heading <- curbcut::cc_t(lang = r$lang(), out$place_heading)
  
  
  ## Geo name ------------------------------------------------------------------
  
  out$geo <- 
    curbcut::cc_t(lang = r$lang(), regions_dictionary$to_compare[regions_dictionary == geo])
  ...
  if (grepl("uni_quant_select", out$var_type)) {
    ...
    out$larger <- if (val_left >= quintile[4]) {
      curbcut::cc_t(lang = r$lang(), "much larger than")
    } else if (val_left >= quintile[3]) {
      curbcut::cc_t(lang = r$lang(), "larger than")
    } else if (val_left >= quintile[2]) {
      curbcut::cc_t(lang = r$lang(), "almost the same as")
    } else if (val_left >= quintile[1]) {
      curbcut::cc_t(lang = r$lang(), "smaller than")
    } else curbcut::cc_t(lang = r$lang(), "much smaller than")

    out$high <- if (str_detect(out$larger, paste(curbcut::cc_t(lang = r$lang(), "much larger than"), 
                                                  curbcut::cc_t(lang = r$lang(), "larger than"), 
                                                  sep = "|"))) {
      curbcut::cc_t(lang = r$lang(), "high")
    } else if (str_detect(out$larger, paste(curbcut::cc_t(lang = r$lang(), "smaller than"), 
                                            curbcut::cc_t(lang = r$lang(), "much smaller than"), 
                                            sep = "|"))) {
      curbcut::cc_t(lang = r$lang(), "low")
    } else curbcut::cc_t(lang = r$lang(), "moderate")
    ...
    out$increase <- if (val_left >= 0) curbcut::cc_t(lang = r$lang(), "increased") else
      curbcut::cc_t(lang = r$lang(), "decreased")
  ...
  if (grepl("qual_", out$var_type)) {
    ...
    out$majority <- if (mode_prop > 0.5) curbcut::cc_t(lang = r$lang(), "majority") else 
      curbcut::cc_t(lang = r$lang(), "plurality")
  ...
  if (grepl("bi_quantxy|date", out$var_type)) {
      ...
      out$pos <- if (corr > 0) curbcut::cc_t(lang = r$lang(), "positive") else 
        curbcut::cc_t(lang = r$lang(), "negative")
      
      out$strong <- if (abs(corr) > 0.6) {
        curbcut::cc_t(lang = r$lang(), "strong")
      } else if (abs(corr) > 0.3) {
        curbcut::cc_t(lang = r$lang(), "moderate")
      } else curbcut::cc_t(lang = r$lang(), "weak")
      
      out$higher <- ifelse(out$pos == curbcut::cc_t(lang = r$lang(), "positive"),
                           curbcut::cc_t(lang = r$lang(), "higher"), curbcut::cc_t(lang = r$lang(), "lower"))
      
      out$high_low_disclaimer <- if (out$strong == curbcut::cc_t(lang = r$lang(), "strong")) {
        curbcut::cc_t(lang = r$lang(), "with only a few exceptions")
      } else if (out$strong == curbcut::cc_t(lang = r$lang(), "moderate")) {
        curbcut::cc_t(lang = r$lang(), "although with some exceptions")
      } else if (out$strong == curbcut::cc_t(lang = r$lang(), "weak")) {
        curbcut::cc_t(lang = r$lang(), "although with many exceptions")
      }
  }
  ...
  if (grepl("bi_quantxy_select", out$var_type)) {
    ...
    out$relative_position <- if (abs(perc_left - perc_right) > 0.5) {
      curbcut::cc_t(lang = r$lang(), "dramatically different")
    } else if (abs(perc_left - perc_right) > 0.3) {
      curbcut::cc_t(lang = r$lang(), "substantially different")
    } else if (abs(perc_left - perc_right) > 0.1) {
      curbcut::cc_t(lang = r$lang(), "considerably different")
    } else curbcut::cc_t(lang = r$lang(), "similar")
  ...
  if (grepl("date_", out$var_type)) {
    ...
    if (length(max_date) %in% 2:3) max_date <- paste(
      ...
      max_date[length(max_date)], sep = curbcut::cc_t(lang = r$lang(), " and "))
    if (length(max_date) > 3) out$max_date <- 
      curbcut::cc_t(lang = r$lang(), "several different dates")
    ...
    if (length(min_date) %in% 2:3) min_date <- paste(
      ...
      max_date[length(min_date)], sep = curbcut::cc_t(lang = r$lang(), " and "))
    if (length(min_date) > 3) min_date <- 
      curbcut::cc_t(lang = r$lang(), "several different dates")
    out$min_date <- min_date
    
    out$coef <- abs(coef)
    out$coef_increasing <- if (coef >= 0) curbcut::cc_t(lang = r$lang(), "increasing") else 
      curbcut::cc_t(lang = r$lang(), "decreasing")
