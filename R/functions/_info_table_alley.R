...
info_table_alley <- function(r = r, data, var_type, var_left, var_right, df, 
  ...
  if (curbcut::is_scale_df("borough_empty", df)) {
    ...
    if (is.na(select_id) || !select_id %in% alley_text$ID) {
      ...
      curbcut::cc_t(lang = r$lang(), "{participating_boroughs} out of 19 Montreal boroughs ",
      ...
      text_to_display$title <- paste0("<p><b>", curbcut::cc_t(lang = r$lang(), "Borough"), " ", 
                                      data$name, "</b></p>")
      text_to_display$intro <-
        paste0("<p>",
               curbcut::cc_t(lang = r$lang(), "The first green alley inauguration was in "),
               data$first_alley, if (!is.na(data$green_alley_sqm)) 
                 curbcut::cc_t(lang = r$lang(), " and there are {data$green_alley_sqm} square",
                               " meters of green alley in the borough.") else ".",
               "</p>")
      text_to_display$text <- 
        paste0("<p>",
               if (!is.na(data$app_process)) curbcut::cc_t(lang = r$lang(), data$app_process), " ",
               if (!is.na(data$management)) curbcut::cc_t(lang = r$lang(), data$management), " ",
               if (!is.na(data$budget)) curbcut::cc_t(lang = r$lang(), data$budget),
               "</p>")
      
      text_to_display$guide <- 
        paste0(glue("<p><a href = {data$guide}>"),
               curbcut::cc_t(lang = r$lang(), "The green alley guide of {data$name}"),
               "</a></p>")
  ...
  } else if (curbcut::is_scale_df("alley", df)) {
    ...
    if (is.na(select_id) || !select_id %in% alley$ID) {
      ...
      curbcut::cc_t(lang = r$lang(), "Our team visited {nrow(alley_visited)} of the ",
      ...
      if (!is.na(data$created)) 
        ...
        paste0("<p>",
               curbcut::cc_t(lang = r$lang(), "Inauguration date: "), 
               data$created, "</p>")
      text_to_display$text <- 
        if (is.na(data$description)) {
          paste0("<p>",
                 curbcut::cc_t(lang = r$lang(), "We do not have information ",
                               "on this green alley."),
                 "</p>")
        } else {
          paste0("<p>", curbcut::cc_t(lang = r$lang(), data$description), "</p>",
                 if (!is.na(data$circulation)) {
                   paste0("<p>Circulation: ", curbcut::cc_t(lang = r$lang(), data$circulation),
                          "</p>")
                 })
        }
