...
map_change <- function(id, map_id, data, df, zoom = df, click = reactive(NULL),
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(data(), if (!selection() %in% data()$ID) selection(NA), 
                 ignoreInit = TRUE)
    ...
    building_to_add <- reactive({
      ...
      if (df() != "building" || is.na(selection())) return(NULL)
    ...
    return(reactive({if (df() == "building") building_id() else selection()}))
