...
place_explorer_block <- function(df, selected_var, select_id) {
  ...
  list(graph = explore_graph(data, var_type, selected_var, " ", df, select_id, 
                build_str_as_DA = TRUE, plot_type = "auto"),
       info = info_table(data, var_type, selected_var, " ", df, select_id))
}