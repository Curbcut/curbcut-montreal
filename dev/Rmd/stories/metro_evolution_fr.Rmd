...
Le réseau de transport en commun à Montréal a connu une transformation majeure depuis le début du 20e siècle et le système de métro en est un aspect central et emblématie. Non seulement l'évolution du métro n'a pas été linéaire, mais elle a aussi révélé les priorités de la ville et la perception qu'elle avait d'elle-même. Voici l’histoire du métro de Montréal, comment il est arrivé à son état actuel et ce que lui réserve l’avenir. 
...
names(metro1910)[1] <- "Légende"
metro1910$Légende[metro1910$Légende=="Proposed extension"] <- "Prolongement proposé"
metro1910$Légende[metro1910$Légende=="Proposed line"] <- "Ligne proposée"
tmap_mode("view")
m <- tm_shape(metro1910)+
  tm_lines("Légende", palette = c(`Prolongement proposé` = "#1263a6", `Ligne proposée` = "black"), 
           lwd = 5)
tmap_leaflet(m)
```
<div align="center">Le réseau proposé en 1910.</div>

Plusieurs propositions semblables ont été faites dans les années suivantes, mais rien de concret n’a émergé au début du 20e siècle. La première guerre mondiale suivie de la Grande Dépression ont empêché l’approbation et le financement d’un projet de métro, car la ville a été plongée dans une crise économique et la congestion s’était améliorée, atténuant les problèmes qui nécessitaient le recours au transport souterrain (Gilbert, 2021). 
...
```{r, echo=FALSE, message=FALSE}
names(metro1944)[1] <- "Légende"
metro1944$Légende[metro1944$Légende=="Proposed extension"] <- "Prolongement proposé"
metro1944$Légende[metro1944$Légende=="Proposed line"] <- "Ligne proposée"
tmap_mode("view")
...
tmap_leaflet(l)
```
<div align="center">Le réseau proposé en 1944.</div>
...
En 1953, la Commission de transport de Montréal, un nouvel organisme ayant pour but de remplacer le réseau de tramway par un réseau de bus (Société de Transport de Montréal, 2004), a proposé un autre projet de métro inspiré du plan de 1944. Pourtant, ce plan n’a pas réussi non plus dépassé la phase de planification. 

```{r, echo=FALSE, message=FALSE}
names(metro1953)[1] <- "Légende"
metro1953$Légende[metro1953$Légende=="Proposed lines"] <- "Lignes proposées"
o <- tm_shape(metro1953)+
  tm_lines("Légende", palette = "black", lwd = 3)
tmap_leaflet(o)
```
<div align="center">Le réseau proposé en 1953.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro1961)[1] <- "Légende"
metro1961$Légende[metro1961$Légende=="Proposed extensions"] <- "Prolongements proposés"
metro1961$Légende[metro1961$Légende=="Proposed orange and green lines"] <- "Lignes orange and verte proposées"
metro1961$Légende[metro1961$Légende=="Proposed red line"] <- "Ligne rouge proposée"
p <- tm_shape(metro1961)+
  tm_lines("Légende", palette = c(`Proposed extensions` = "#1263a6", 
                               `Proposed orange and green lines` = "black", 
                               `Proposed red line` = "red"),
           lwd = 3)
tmap_leaflet(p)
```
<div align="center">Le réseau de métro proposé en 1962, qui s'agissait de trois lignes: orange, verte et rouge.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro1967)[1] <- "Légende"
metro1967$Légende[metro1967$Légende=="Green line"] <- "Ligne verte"
metro1967$Légende[metro1967$Légende=="Orange line"] <- "Ligne orange"
metro1967$Légende[metro1967$Légende=="Yellow line"] <- "Ligne jaune"
q <- tm_shape(metro1967)+
  tm_lines("Légende", palette = c(`Ligne verte` = "#00A650", `Ligne orange` = "#F47216", `Ligne jaune` = "#FCD300"),
           lwd = 3)
tmap_leaflet(q)
```
<div align="center">Le réseau de métro réalisé en 1967.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro1981)[1] <- "Légende"
metro1981$Légende[metro1981$Légende=="Orange line"] <- "Ligne orange"
metro1981$Légende[metro1981$Légende=="Green line"] <- "Ligne verte"
metro1981$Légende[metro1981$Légende=="Yellow line"] <- "Ligne jaune"
metro1981$Légende[metro1981$Légende=="Proposed orange line extension"] <- "Prolongement de la ligne orange proposé"
metro1981$Légende[metro1981$Légende=="Proposed green line extension"] <- "Prolongement de la ligne verte proposé"
metro1981$Légende[metro1981$Légende=="Proposed blue line"] <- "Ligne bleue proposée"
r <- tm_shape(metro1981)+
  tm_lines("Légende", palette = c(`Ligne orange` = "#F47216",
                               `Ligne verte` = "#00A650",
                               `Ligne jaune` = "#FCD300",
                               `Prolongement de la ligne orange proposé` = "#f5d7a4",
                               `Prolongement de la ligne verte proposé` = "#76c274",
                               `Ligne bleue proposée` = "#9cc0f0"),
           lwd = 3)
tmap_leaflet(r)
```
<div align="center">Les plans pour l'expansion du métro faits à la suite de la réélection de Jean Drapeau.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro1978)[1] <- "Légende"
metro1978$Légende[metro1978$Légende=="Orange line"] <- "Ligne orange"
metro1978$Légende[metro1978$Légende=="Green line"] <- "Ligne verte"
metro1978$Légende[metro1978$Légende=="Yellow line"] <- "Ligne jaune"
s <- tm_shape(metro1978)+
  tm_lines("Légende", palette = c(`Ligne verte` = "#00A650",
                               `Ligne orange` = "#F47216",
                               `Ligne jaune` = "#FCD300"),
           lwd = 3)
tmap_leaflet(s)
```
<div align="center">Le réseau de métro en 1978.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro1970s)[1] <- "Légende"
metro1970s$Légende[metro1970s$Légende=="Orange line"] <- "Ligne orange"
metro1970s$Légende[metro1970s$Légende=="Blue line"] <- "Ligne bleue"
metro1970s$Légende[metro1970s$Légende=="Green line"] <- "Ligne verte"
metro1970s$Légende[metro1970s$Légende=="Yellow line"] <- "Ligne jaune"
metro1970s$Légende[metro1970s$Légende=="Proposed orange line extension"] <- "Prolongement de la ligne orange proposé"
metro1970s$Légende[metro1970s$Légende=="Proposed blue line extension"] <- "Prolongement de la ligne bleue proposé"
metro1970s$Légende[metro1970s$Légende=="Proposed line 10"] <- "Ligne 10 proposée"
metro1970s$Légende[metro1970s$Légende=="Proposed line 11"] <- "Ligne 11 proposée"
metro1970s$Légende[metro1970s$Légende=="Proposed line 6"] <- "Ligne 6 proposée"
metro1970s$Légende[metro1970s$Légende=="Proposed line 7"] <- "Ligne 7 proposée"
metro1970s$Légende[metro1970s$Légende=="Proposed line 8"] <- "Ligne 8 proposée"
n <- tm_shape(metro1970s)+
  tm_lines("Légende", palette = c(`Ligne bleue` = "#1082CD", 
                               `Ligne orange` = "#F47216",
                               `Ligne verte` = "#00A650",
                               `Ligne jaune` = "#FCD300",
                               `Prolongement de la ligne orange proposé` = "#f5d7a4",
                               `Prolongement de la ligne bleue proposé` = "#9cc0f0",
                               `Ligne 10 proposée` = "black",
                               `Ligne 11 proposée` = "#454545",
                               `Ligne 6 proposée` = "#696868",
                               `Ligne 7 proposée` = "#949292",
                               `Ligne 8 proposée` = "#b5b1b1"),
           lwd = 3)
tmap_leaflet(n)
```
<div align="center">Des expansions de surface et souterraines planifiées dans les années soixante-dix jamais réalisées.</div>
...
```{r, echo=FALSE, message=FALSE}
names(metro2000s)[1] <- "Légende"
metro2000s$Légende[metro2000s$Légende=="Orange line"] <- "Ligne orange"
metro2000s$Légende[metro2000s$Légende=="Blue line"] <- "Ligne bleue"
metro2000s$Légende[metro2000s$Légende=="Green line"] <- "Ligne verte"
metro2000s$Légende[metro2000s$Légende=="Yellow line"] <- "Ligne jaune"
metro2000s$Légende[metro2000s$Légende=="Proposed orange line extension"] <- "Prolongement de la ligne orange proposé"
metro2000s$Légende[metro2000s$Légende=="Blue line extension"] <- "Prolongement de la ligne bleue"
metro2000s$Légende[metro2000s$Légende=="Proposed yellow line extension"] <- "Prolongement de la ligne jaune proposé"
metro2000s$Légende[metro2000s$Légende=="Proposed pink line"] <- "Ligne rose proposée"
t <- tm_shape(metro2000s)+
  tm_lines("Légende", palette = c(`Ligne orange` = "#F47216",
                               ...
                               `Ligne jaune` = "#FCD300",
                               `Prolongement de la ligne orange proposé` = "#f5d7a4",
                               `Prolongement de la ligne bleue` = "#9cc0f0",
                               `Prolongement de la ligne jaune proposé` = "#faf093",
                               `Ligne rose proposée` = "#E60E70"),
           lwd = 3)
tmap_leaflet(t)
```
<div align="center">Le réseau moderne avec les changements proposés au fil des années, parmi eux le prolongement confirmé de la ligne bleue vers Anjou.</div>
