...
build_and_append_afford <- function(scales_variables_modules, crs) {

  # Read and prepare data ---------------------------------------------------

  # Read the data placed in the centraide folder
  csvs <- list.files("dev/data/centraide/pop", full.names = TRUE)
  ...
  names(data)[2:ncol(data)] <- paste0("afford_", names(data)[2:ncol(data)])
  ...
  parent_strings <- lapply(unique_vars, \(x) "afford_total_total_total_total_total_count")
  ...
  variables <- lapply(unique_vars, \(var) {
    ...
    # Auto variables
    group_name <- 
      if (grepl("_count$", var)) "Number of individuals" else "Percent of all individuals"
    group_diff <- list(
      "Tenure status" = (\(x) {
  ...
  modules <-
    ...
    add_module(
      ...
      dates = with_breaks$avail_dates[["afford_total_total_total_total_total_pct"]],
      var_right = var_right,
