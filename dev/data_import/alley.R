...
build_and_append_alley <- function(scales_variables_modules, scales_sequences, crs,
                                   city_scales_ID, inst_prefix) {
  
  if (!file.exists("data/alleys.qsm")) {
    ...
    boroughs <- scales_variables_modules$scales$borough[c("ID", "name")] |>
      sf::st_drop_geometry()
  ...
  city_data <- 
    sapply(c("borough", "CT", "DA"), \(scale) {
      
      data <- scales_variables_modules$scales[[scale]]["ID"]
      data <- data[data$ID %in% city_scales_ID[[scale]], ]
      ...
      data <- scales_variables_modules$scales[[scale]][c("ID", "population")]
      data <- sf::st_transform(data, crs)
  ...
  data_interpolated$scales$borough <- 
    merge(data_interpolated$scales$borough, city_data$borough, by = "ID", all.x = TRUE)
  data_interpolated$scales$CT <- 
    merge(data_interpolated$scales$CT, city_data$CT, by = "ID", all.x = TRUE)
  data_interpolated$scales$DA <- 
    merge(data_interpolated$scales$DA, city_data$DA, by = "ID", all.x = TRUE)
  ...
  variables <-
    add_variable(
      ...
      private = FALSE,
      dates = "2023",
      avail_scale = c("borough", "CT", "DA"),
      source = "Curbcut",
      interpolated = tibble::tibble(scale = c("borough", "CT", "DA"),
  ...
  modules <-
    ...
    add_module(
      ...
      var_left = c("alley_sqkm", "alley_per1k"),
      default_var = "alley_sqkm",
      dates = 2023,
      metadata = TRUE,
      dataset_info = paste0(
