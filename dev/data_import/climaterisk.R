...
build_and_append_climate_risk <- function(scales_variables_modules, crs,
  ...
  # 
  # # Loop through every column to change 0s to NAs
  # for(i in 2:10) {
  ...
  if (length(only_scales_exc) != 0) {
    ...
    data_interpolated <-
      interpolate_custom_geo(data = climate_risk,
                             ...
                             only_scales = names(scales_variables_modules$scales)[
                               !grepl("^grd|building", names(scales_variables_modules$scales))
                             ],
                             construct_for = names(scales_variables_modules$scales)[
                               !grepl("^grd|building", names(scales_variables_modules$scales))
                             ],
                             crs = crs,
                             overwrite = overwrite,
    ...
    data_interpolated$scales$grd25 <- 
      merge(scales_variables_modules$scales$grd25,
    ...
    data_interpolated$scales$grd50 <- 
      merge(scales_variables_modules$scales$grd50,
    ...
    data_interpolated$scales$grd100 <- 
      merge(scales_variables_modules$scales$grd100,
    ...
    data_interpolated$scales$grd250 <- 
      merge(scales_variables_modules$scales$grd250,
  ...
  interpolated_df <- tibble::tibble(scale = only_scales,
                                    interpolated_from = "grd25")
  interpolated_df$interpolated_from[interpolated_df$scale == "grd25"] <- FALSE
