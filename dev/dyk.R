...
dyk <-
  ...
  mutate(left_variable = str_replace(left_variable, "_prop", "_pct")) |> 
  mutate(right_variable = str_replace(right_variable, "_prop", "_pct"))
...
category_table <- 
  dyk |> 
  unnest(c(category, variable)) |> 
  distinct(category, variable)
