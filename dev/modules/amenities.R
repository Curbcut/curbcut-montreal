...
# 
#   p <-
#     progressr::progressor(ceiling(length(DA_amenities) *
#                                     length(tt_matrix_DA) *
#                                     (length(tt_matrix_DA[[3]])/4) *
...
#             reduce(threshold, left_join, by = "fromId")
#           } else threshold
...
# DA_amenities <-
#   reduce(DA_amenities, left_join, by = "fromId") |>
#   rename_with(~paste0("amenities_", .x)) |>
#   rename(ID = amenities_fromId)
# 
# names(DA_amenities) <- gsub("__", "_", names(DA_amenities))
# 
