...
process_census_data <- function(data) {
  ...
  data <- 
    ...
    select(-c(renter, tenure_households, value_avg_total, major_repairs,
              housing_stress_renter, housing_stress_owner, rent_avg_total, housing_mobility_one,
    ...
    select(-c(emp_professional, emp_management, emp_cultural, emp_labour_total,
              emp_arts)) %>% 
    # education
    mutate(edu_bachelor_above_prop = edu_bachelor_above / (edu_total + edu_15_19_male + edu_15_19_female),
           edu_no_degree_prop = edu_bachelor_above / (edu_total + edu_15_19_male + edu_15_19_female)) %>% 
    select(-c(edu_bachelor_above, edu_no_degree, edu_total, edu_15_19_male, edu_15_19_female)) %>% 
    # other processing
