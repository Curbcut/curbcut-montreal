...
build_and_append_climate_risk <- function(scales_variables_modules, crs) {
  ...
  climate_risk <- 
    map2(list.files("dev/data/climate_risk", "*.shp$", full.names = TRUE), 
         c("climate_flood", "climate_heavy_rain", "climate_drought", 
  ...
  data_interpolated$scales$grid$grid <- 
    merge(scales_variables_modules$scales$grid$grid, 
          sf::st_drop_geometry(climate_risk), by = "ID")
  data_interpolated$avail_scales[nrow(data_interpolated$avail_scales) + 1, ] <- 
    tibble::tibble(geo = "grid", scale = "grid")
  data_interpolated$interpolated_ref[nrow(data_interpolated$interpolated_ref) + 1, ] <- 
    tibble::tibble(geo = "grid", scale = "grid", interpolated_from = "FALSE")
  ...
  with_breaks$scales$grid$grid$climate_flood_q5 <- 
    with_breaks$scales$grid$grid$climate_flood
  with_breaks$scales$grid$grid$climate_heavy_rain_q5 <- 
    with_breaks$scales$grid$grid$climate_heavy_rain
  with_breaks$scales$grid$grid$climate_destructive_storms_q5 <- 
    with_breaks$scales$grid$grid$climate_destructive_storms
  with_breaks$scales$grid$grid$climate_heat_wave_q5 <- 
    with_breaks$scales$grid$grid$climate_heat_wave
